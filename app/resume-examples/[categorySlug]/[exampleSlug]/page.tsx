        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-xl text-blue-100 max-w-3xl mx-auto mb-6">
              Professional {category.name.toLowerCase()} resume template with expert writing tips
            </p>
            <div className="flex justify-center gap-4 text-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {example.writingTips && example.writingTips.length > 0 && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-8">
                ...
                <ul className="space-y-3">
                  {example.writingTips.map((tip, index) => (
                    <li key={index} className="flex items-start gap-3">
                      <span className="text-amber-600 font-bold mt-1">{index + 1}.</span>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blue-600 text-white rounded-xl p-6">
              ...
              <p className="text-blue-100 mb-4">Pre-fill your resume with this professional example.</p>
              <Link
                href={`/builder?template=${example.slug}&category=${category.slug}`}
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900">{experienceLevelLabels[example.experienceLevel]}</p>
                </div>
                {example.salaryRange && (
                  <div>
                    <span className="text-sm text-gray-500">Salary Range</span>
                    <p className="font-medium text-gray-900">{example.salaryRange}</p>
                  </div>
                )}
                {example.jobOutlook && (
                  <div>
                    <span className="text-sm text-gray-500">Job Outlook</span>
                    <p className="font-medium text-gray-900">{example.jobOutlook}</p>
                  </div>
                )}
              </div>
            </div>
...
// app/resume-examples/[categorySlug]/[exampleSlug]/page.tsx
import { Metadata } from 'next';
import Link from 'next/link';
import { notFound } from 'next/navigation';
import { prisma } from '@/lib/prisma';
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blue-600 text-white rounded-xl p-6">
              ...
              <p className="text-blue-100 mb-4">
                Pre-fill your resume with this professional example.
              </p>
              <Link
                href={`/builder?template=${example.slug}&category=${category.slug}`}
                className="inline-flex items-center gap-2 bg-white text-blue-600 font-semibold px-6 py-3 rounded-lg hover:bg-blue-50 transition-colors w-full justify-center"
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <div className="space-y-3">
                  {relatedExamples.map((related) => (
                    <Link
                      key={related.id}
                      href={`/resume-examples/${category.slug}/${related.slug}`}
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        {experienceLevelLabels[related.experienceLevel]}
                      </p>
                    </Link>
                  ))}
                </div>
